...
class BACnetIOHandler(NormalApplication, ForeignApplication):
	...
	async def read_multiple_device_props(self, apdu) -> bool:
		try:  # Send readPropertyMultiple and get response
			...
			logging.error(
				f"Error PDU reading device props: {device_identifier}: {err}"
			)
	...
	async def read_multiple_object_list(self, device_identifier):
		...
		for obj_id in self.bacnet_device_dict[f"device:{device_identifier[1]}"][
			...
			try:  # Send readPropertyMultiple and get response
				...
				logging.error(f"Nack error while reading object list: {device_identifier}: {obj_id} {err}")
				...
				logging.error(f"Assertion error for: {device_identifier}: {obj_id} {err}")
			...
			for obj_id in self.bacnet_device_dict[f"device:{device_identifier[1]}"][
				...
				for property_id in object_properties_to_read_once:
					try: 
						...
						logging.error(f"Nack error reading object list one by one: {device_identifier} {obj_id} {property_id}: {err}")
						continue
					else:
						if response and response is not ErrorType:
							self.dict_updater(
		...
		for obj_id in self.bacnet_device_dict[device_identifier]:
			...
			for property_id in object_properties_to_read_periodically:
				try:    
					...
					logging.error(f"Error PDU reading objects one by one periodically: {device_identifier} {obj_id} {property_id}: {err}")
					continue

				except ErrorRejectAbortNack as err:
					logging.error(f"Nack error reading objects one by one periodically: {device_identifier} {obj_id} {property_id}: {err}")
					continue
				except AttributeError as err:
					logging.error(f"Attribute error: {obj_id}: {err}")
				else:
