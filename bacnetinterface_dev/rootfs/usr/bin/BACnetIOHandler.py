        ...
        async with self.read_semaphore:
            ...
            try:
                ...
                if "segmentation-not-supported" in str(err):
                    LOGGER.debug(
                        f"Segmentation not supported: {device_identifier} {object_identifier} {err}"
                    )
                    ...
                    LOGGER.debug(
                        f"No ReadMultiple: {device_identifier} {object_identifier} {err}"
                    )
                    return await self.properties_read(
                        device_identifier, object_identifier, property_list
                    )
                    ...
                    LOGGER.debug(
                        f"No response: {device_identifier} {object_identifier} {err}"
                    )
                    return False
                else:
                    LOGGER.debug(
                        f"Other error: {device_identifier} {object_identifier} {err}"
                    )
                    return False
            ...
            async with self.read_semaphore:
                try:
                    ...
                    if "segmentation-not-supported" in str(err):
                        ...
                        LOGGER.debug(
                            f"Unknown property: {device_identifier} {object_identifier} {err}"
                        )
                        return True
                    elif "no-response" in str(err):
                        LOGGER.debug(
