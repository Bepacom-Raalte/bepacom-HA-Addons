...
async def refresh_subscription_wrapper(self):
	device_identifier = self.app.addr_to_dev(self.address)
...
class BACnetIOHandler(
	...
	async def discover_devices(self):
		...
		for config in generated_configs + retrieved_configs:
			if config in generated_configs:
				await self.config_to_tasks(configuration=config, first_run=True)
			else:
				await self.config_to_tasks(configuration=config, first_run=False)
	...
	def retrieve_config(
		...
		if not configuration:
			return self.generate_config(device_identifier)
	...
	async def get_property_list(
		...
		config: DeviceConfiguration = self.retrieve_config(device_identifier)
	...
	async def properties_read_multiple(
		...
		try:
			async with config.semaphore:  # Limit concurrency only for read_property_multiple
	...
	async def properties_read(
		...
		async def read_property_safely(property_id: PropertyIdentifier):
			...
			try:
				async with config.semaphore:  # Limit concurrency only for read_property
	...
	async def read_list_property(
		...
		try:
			...
			async def read_with_semaphore(number):
				async with config.semaphore:  # Acquire semaphore per task
					response =  await self.read_property(
						address=self.dev_to_addr(device_identifier),
						objid=object_identifier,
						prop=property_id,
