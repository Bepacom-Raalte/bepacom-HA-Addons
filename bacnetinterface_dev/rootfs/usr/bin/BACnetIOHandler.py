...
class BACnetIOHandler(
	...
	async def config_to_tasks(self, configuration: DeviceConfiguration):
		...
		object_list = [item for item in configuration.cov_items if ObjectType(item[0]) in subscribable_objects]
	...
	async def get_property_list(self, device_identifier, object_identifier, fallback_list) -> list[PropertyIdentifier]:
		try:
	...
	async def properties_read_multiple(self, device_identifier: ObjectIdentifier, object_identifier: ObjectIdentifier, property_list: list[PropertyIdentifier]) -> bool:
		...
		try:
			...
			if "segmentation-not-supported" in str(err):
				LOGGER.debug(f"Segmentation not supported: {device_identifier} {object_identifier} {err}")
				return await self.properties_read(device_identifier, object_identifier, property_list)
			elif "unrecognized-service" in str(err):
				LOGGER.debug(f"No ReadMultiple: {device_identifier} {object_identifier} {err}")
				return await self.properties_read(device_identifier, object_identifier, property_list)
			elif "no-response" in str(err):
				LOGGER.debug(f"No response: {device_identifier} {object_identifier} {err}")
				return False
			else:
	...
	async def properties_read(self, device_identifier: ObjectIdentifier, object_identifier: ObjectIdentifier, property_list: list[PropertyIdentifier]) -> bool:
		...
		async def read_property_safely(property_id: PropertyIdentifier):
			...
			try:
				...
				if "segmentation-not-supported" in str(err):
					LOGGER.debug(f"Segmentation not supported: {device_identifier} {object_identifier} {err}")
					return await self.read_list_property(device_identifier, object_identifier, property_id)
				elif "unknown-property" in str(err):
					LOGGER.debug(f"Unknown property: {device_identifier} {object_identifier} {err}")
					return True
				elif "no-response" in str(err):
					LOGGER.debug(f"No response: {device_identifier} {object_identifier} {err}")
					return False
	...
	async def read_list_property(self, device_identifier: ObjectIdentifier, object_identifier: ObjectIdentifier, property_id: PropertyIdentifier) -> bool:
		...
		LOGGER.debug(f"Read list property: {device_identifier} {object_identifier} {property_id}")
		...
		try:
			...
			if object_amount == 0:
				LOGGER.debug(f"Amount is zero: {device_identifier} {object_identifier} {property_id}")
				return False
		except ErrorRejectAbortNack as err:
			LOGGER.warning(
	...
	def is_in_dict(self, device_identifier: ObjectIdentifier, object_identifier: ObjectIdentifier) -> bool:
	...
	async def object_poll_task(self, device_identifier: ObjectIdentifier, object_identifier: ObjectIdentifier, poll_rate: int | float = 300):
		...
		try:
			...
			if (
				...
				await self.write_to_api_queue.put(
					(object_identifier, property_type, array_index, priority, property_value)
				)
